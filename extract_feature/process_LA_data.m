...
pathToASVspoof2019Data = '/data/users/XX/LA/';
pathToFeatures = horzcat('/data/users/YY/LAmat');
% mkdir(pathToFeatures);

% pathToDatabase = fullfile(pathToASVspoof2019Data, access_type);
pathToDatabase = fullfile(pathToASVspoof2019Data);
...
for i=1:length(trainfilelist)
    ...
    mkdir(fullfile(pathToFeatures, 'train'));
...
for i=1:length(devfilelist)
    ...
    [stat,delta,double_delta] = extract_lfcc(x,fs,20,512,20);
    LFCC = [stat delta double_delta]';
    mkdir(fullfile(pathToFeatures, 'dev'));
...
for i=1:length(evalfilelist)
    ...
    [stat,delta,double_delta] = extract_lfcc(x,fs,20,512,20);
    LFCC = [stat delta double_delta]';
    mkdir(fullfile(pathToFeatures, 'eval'));
